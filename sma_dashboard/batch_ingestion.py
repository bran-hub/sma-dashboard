...
def ingest_manifest(
    ...
    for entry in entries:
        if replace_date:
            _delete_existing_model_update(db_path, entry.model_date)
...
def _delete_existing_model_update(db_path: Path | str, model_date: str) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--replace-date",
        action="store_true",
        help="Before each manifest row, delete existing holdings/trades for that model_date.",
    )
